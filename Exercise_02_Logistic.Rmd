...
for(r in dif_rs){
  ...
  title(main = paste("Population Growth (r =", r, ")"))
}

```

As R increases the graphs become more chaotic. For r = 1.95 you see a relatively stable end at 10. However, for r = 2.8, the end varies a lot more.
...
x = seq(-10,50,by = 1)
plot(x,dpois(x,lambda = 5),type='s') 

## CDF Function
plot(x,ppois(x,lambda = 5),type='s')

## Quantile
p = seq(0,1,by=0.01)
plot(p,qpois(p,lambda = 5),type='l')

## Histogram samples
n = c(10,100,1000,10000)	# sequence of sample sizes
for(i in 1:4){			# loop over these sample sizes
  hist(rpois(n[i], lambda = 5),main=n[i],probability=TRUE,breaks=x)  
				#here breaks defines number of bins in the histogram
  lines(x-1,dpois(x, lambda = 5),col=2, type='s')
}
...
print(paste("Mean of x: ", as.character(mean(x))))
print(paste("Median of x: ", as.character(median(x))))
print(paste("95% CI of x: ", as.character(quantile(x,.95))))

print(paste("Mean of y: ", as.character(mean(y))))
print(paste("Median of y: ", as.character(median(y))))
print(paste("95% CI of y: ", as.character(quantile(y,.95))))
...
```{r}
hist(rE, main = "Histogram of r", xlab = "r", col = "pink")
hist(KE, main = "Histogram of K", xlab = "K", col = "lightgreen")
```
...
```{r}
## Using a for loop
plot(1:NT, n[1, ], col = 1, type = "l", ylim = range(apply(n, 2, range)), main = "Sample Trajectories", xlab = "time", ylab = "n")

for(x in 1:10){
  lines(1:NT, n[x,], type = "l", col = x, ylim = range(apply(n, 2, range)))
}

## Online Help Solution
matplot(1:NT, t(n[1:10, ]), type = "l", col = "darkblue", ylim = range(apply(n, 2, range)), main = "Sample Trajectories", xlab = "time", ylab = "n")
```

6. [B] Plot a histogram of your population forecast at time = 15.

```{r}
hist(n[, 15], col = "darkorchid", main = "Histogram forecase at t = 15", xlab = "n")
```
...
plot(1:NT, apply(n, 2, median), type = "l", col = "darkred", 
  ylim =  range(apply(n, 2, range)), main = "Sample Trajectories", xlab = "time", ylab = "n")

ciEnvelope(1:NT, n.stats[1, ], n.stats[3, ], col = rgb(1, 0.6, 0.8, alpha = 0.25))
...
```{r}
plot_med_95 <- function(r, n, NT){
  initial_con(r)
  
  plot(1:NT, apply(n, 2, median), type = "l", col = "darkred", 
  ylim =  range(apply(n, 2, range)), main = 
  paste("Initial Conditions (r =", r, ")"), xlab = "time", ylab = "n")
  
  n.stats = apply(n,2,quantile,c(0.025,0.5,0.975))

  ciEnvelope(1:NT, n.stats[1, ], n.stats[3, ], col = rgb(1, 0.6, 0.8, 
                                                         alpha = 0.25))
}

plot_med_95(0.3, n, NT)
```

9. [C] Repeat with r equal to 1.95, 2.05, and 2.8
```{r}
for(r in c(1.95,2.05,2.8)){
  plot_med_95(r, n, NT)
}
```
