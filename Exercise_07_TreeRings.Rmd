...
n.iter = 10000                           ## INCREASE THIS NUMBER FOR ACTUAL ANALYSES **************************************
...
## this code fuses forest inventory data with tree growth data (tree ring or dendrometer band)
## for the same plots. Code is a rewrite of Clark et al 2007 Ecol Appl into JAGS
TreeDataFusionMV_RandomEffects = "
model{
...
for(i in 1:ni){
  ...
  ## compile JAGS model
  j.model   <- jags.model (file = textConnection(TreeDataFusionMV_RandomEffects),
                           data = data,
                           inits = init,
                           n.chains = 3)
  ## note: this will give a WARNING (not an error) about unused tau_ind, tau_yr, ind, yr 
  ...
  ## burn-in
  jags.out.random   <- coda.samples (model = j.model,
                              variable.names = c("tau_add","tau_dbh","tau_inc","mu","tau_ind","tau_yr"),
                              n.iter = min(n.iter,2000))
  ## note: will also give a WARNING about tau_ind, tau_yr
  plot(jags.out.random)
...
parse.MatrixNames <- function(w, pre = "x", numeric = FALSE) {
  ...
  ### DBH
  layout(matrix(1:8,4,2)) ## arrange plots in a 4 x 2 matrix
  out.random <- as.matrix(jags.out.random)
  x.cols = which(substr(colnames(out.random),1,1)=="x")   ## which columns are the state variable, x
  ci <- apply(out.random[,x.cols],2,quantile,c(0.025,0.5,0.975))
  ci.names = parse.MatrixNames(colnames(ci),numeric=TRUE)
  ...
  for(i in smp){
    sel = which(ci.names$row == i)
    inc.mcmc = apply(out.random[,x.cols[sel]],1,diff)
    inc.ci = apply(inc.mcmc,1,quantile,c(0.025,0.5,0.975))*5
...
```{r}
  ## process model
  vars = (1:ncol(out.random))[-c(which(substr(colnames(out.random),1,1)=="x"),grep("tau",colnames(out.random)), grep("year",colnames(out.random)),grep("ind",colnames(out.random)))] ## remove x, tau's, yr, and ind from plots
  par(mfrow=c(1,1))
  for(i in vars){
    hist(out.random[,i],main=colnames(out.random)[i])
  }
  if(length(vars)>1) pairs(out.random[,vars])

  tau_means = rep(NA, 5)
  
  ## Standard Deviations
  par(mfrow=c(2,3))
  prec = out.random[,grep("tau",colnames(out.random))] ## select just taus to plot
  for(i in 1:ncol(prec)){
    hist(1/sqrt(prec[,i]),main=colnames(prec)[i])
    tau_means[i] = mean(1/sqrt(prec[,i]))
  }
  print(tau_means)
  cor(prec)
  pairs(prec)
...
Tau add (process) was bigger which means there is less unexplained variability in the model. This means precision increased and variance decreased. 
...
Most of the error is in tau_dbh at a mean of 0.57. tau_add and tau_inc have similar means at roughly 0.096. tau_ind is slightly higher than tau_yr with means of 0.133 and 0.123 respectively. 

* What does the size of these effects suggest about the drivers of uncertainty in tree growth? [B]

Most of the error is in the diameter measurement. This makes sense as taking a measuring tape to a tree is a very difficult way to accurately gain data. Additionally, more uncertainty is due to individual tree growth than year to year tree growth.
...
I would add an additional effect to explain individual variability. An example could be something along the lines of species.
...
I would most likely try and see how correlated species variablity is with individual variablility. I would plot species vs individual and see how the scatter plot turns out.
...
for(i in 1:ni){
  ...
  #### Process Model
  for(t in 2:nt){
    Dnew[i,t] <- x[i,t-1] + mu + ind[i] + year[t] + beta[species[i]]
    x[i,t]~dnorm(Dnew[i,t],tau_add)
  }
...
```{r}
  ...
  par(mfrow=c(1,1))
  ### YEAR
  year.cols = grep("year",colnames(out.random))
  if(length(year.cols>0)){
    ci.yr <- apply(out.random[,year.cols],2,quantile,c(0.025,0.5,0.975))
  ...
  ### INDIV
  ind.cols= which(substr(colnames(out.random),1,3)=="ind")
  if(length(ind.cols)>0){
    boxplot(out.random[,ind.cols],horizontal=TRUE,outline=FALSE,col=combined$PLOT,main="Individual Effects By Plot",xlab="cm")
    abline(v=0,lty=2)
    ## calculate plot-level means for random effects
    tapply(apply(out.random[,ind.cols],2,mean),combined$PLOT,mean)
    table(combined$PLOT)
    
    spp = as.factor(combined$SPP)
    boxplot(out.random[order(spp),ind.cols],horizontal=TRUE,outline=FALSE,col=spp[order(spp)],main="Individual Effects By Species",xlab="cm")
    abline(v=0,lty=2)
    ...
    tapply(apply(out.random[,ind.cols],2,mean),combined$SPP,mean)
  }
```
