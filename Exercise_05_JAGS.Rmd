...
```{r}
# Visually look at where the data converges
BGR <- gelman.plot(jags.out)
...
densplot(jags.burn, main = "Marginal Distributions of Parameters")
...
effective_size2 = effectiveSize(jags.burn)
effective_size2
```

-   Describe and explain the parameter covariances that you observe in the pairs plot and parameter correlation matrix.[B]

Based on the Pairwise Parameter Correlation Plots, b1 and b2 are relatively correlated. As seen in the summary table, they have a correlation of roughly -0.88.
...
In this case, they're all the same. The more data you have and the weaker your priors are the more likely they are to be the same. (Monday lecture)
...
```{r}
# Load and create a list of the data
load("data/Ex05.Part1.RData")
data2 <- list(x = x1, y = y, n = length(y))

# Set up new data columns (different from task 1)
...
# Initial Conditions
inits2 = list(S=1/var(y))
inits2 <- list()
inits2[[1]] <- list(S = 1/200)
inits2[[2]] <- list(S = 1/70)
inits2[[3]] <- list(S = 1/20)
...
# Marginal Distribution Plot
densplot(jags.burn2, main = "Marginal Distributions of Parameters")

# Pairwise Parameter Correlations
out <- as.matrix(jags.burn2)
pairs(out)	
cor(out)

# Effective Sample Size
effective_size2 = effectiveSize(jags.burn2)
effective_size2
