...
out.RW = out <- as.matrix(jags.out)         ## convert from coda to matrix  
...
```{r}
# Change the last 40 observations to NA
gflu_new = gflu
gflu_new[580:620,] = NA
y_new = gflu_new$Massachusetts

# Fit the model again
data_new <- list(y=log(y_new),n=length(y_new),      ## data
             x_ic=log(1000),tau_ic=100, ## initial condition prior
             a_obs=1,r_obs=1,           ## obs error prior
             a_add=1,r_add=1            ## process error prior
             )
...
init_new <- list()
for(i in 1:nchain){
  y.samp = sample(y_new,length(y_new),replace=TRUE)
  init_new[[i]] <- list(tau_add=1/var(diff(log(y.samp))),  ## initial guess on process precision
                    tau_obs=5/var(log(y.samp)))        ## initial guess on obs precision
}
...
out.RWF = out <- as.matrix(jags.out)         ## convert from coda to matrix  
x.cols <- grep("^x",colnames(out)) ## grab all columns that start with the letter x
ci <- apply(exp(out[,x.cols]),2,quantile,c(0.025,0.5,0.975)) ## model was fit on log scale

# Summary Stats
jags.burn <- window(jags.out)
summary(jags.burn)
...
ecoforecastR::ciEnvelope(time,ci[1,],ci[3,],col=ecoforecastR::col.alpha("lightBlue",0.75))
points(time,y,cex=0.5, pch="+", col = 'red') # original points turned into NA
points(time,y_new,pch="+",cex=0.5)

legend("topleft", legend = c("NA Points", "Original Data"), pch = "+", col = c("red", "black"), cex = 1)
...
Most of the data points stayed inside of the confidence interval, but not all of them. This means the model was not accurate. Additionaly, the confidence interval was very large so it wasn't very precise. This can be improved by...
...
```{r}
NA_vector = rep(NA, nrow(gflu)) # create a vector of NA's as a base
indices_to_convert <- seq(1, nrow(gflu), by = 4) # select all rows that you want to keep the values for

for(i in indices_to_convert){
  
  # Change the value from NA to the number we wanted to keep
  NA_vector[i] <- gflu[i, "Massachusetts"] 
}

# Set y_new as this new vector with 3/4 NA
y_new = NA_vector
...
The CI for this second run was mush better. While the CI was wider at all points, none of the points went outside the CI. Additionaly, the end didn't have a massive spread of what the values could be.
...
This is when the line of the predicted (median) vs observed plot is a 1:1 line correct? And so if it deviates from that it would be not very accurate? but if all of the points deviate at the same place it would be precise?

* How does the reduction in data volume affect the parameter estimates (taus)

How do I find the taus?
...
Process error (tau_add) SD = 0.2076798 (random walk) 0.19557408 (covariate)
Observation error (tau_obs) SD = 0.1271641 (random walk) 0.06595244 (covariate)

```{r}
## random walk
taus.RW = which(grepl("tau",colnames(out.RW)))
tau.sd.RW = apply(out.RW[,taus.RW],2,function(x){mean(1/sqrt(x))})
tau.sd.RW
exp(tau.sd.RW)

## model with covariates
params.out = as.matrix(params)
taus = grepl("tau",colnames(params.out))
tau.sd = apply(params.out[,taus],2,function(x){mean(1/sqrt(x))})
tau.sd
exp(tau.sd)

sum(tau.sd^2)/sum(tau.sd.RW^2)
```


A decrease in process error means that the model improved. The process error decreased from 0.2076798 to 0.19557408. It decreased to 71% of the original value.
...
beta_0 is the intercept. This represents the expected value when all predictor values are 0. In this model this is the expected level of influenza when Tmin is 0.

beta_1 is the slope of the covariate effect. This represents how much the expected value changes with Tmin. This means that if the value is negative, a lower Tmin indicates a higher influenza. This makes sense as the cold weather seems to be flu season.

beta_X is the slope of the initial condition effect. beta_X is helping us figure out how much our initial conditions change the outcome of the model. beta_X could be any other factors that we didn't think about that influence influenza.

A positive correlation between beta_1 and beta_X indicates that Tmin also impacts the initial conditions.
...
One method is the K-fold cross-validation. I would assume it to be best to choose the blocks randomly as to not input your personal bias. K-fold cross-validation uses different test groups to see if the model is effective. You divide your data into groups and use 1 group as the test group and the rest for fitting. This is repeated for all groups and then the variations are averaged and discussed.
...
## fit the model
data3  <- data
data3$y[length(data3$y)-0:39] = NA
ef.out3 <- ecoforecastR::fit_dlm(model=list(obs="y",fixed="~ 1 + X + Tmin"),data3)


# Visualize
time.rng = c(length(time)-80,length(time))       ## adjust to zoom in and out
x.cols <- grep("^x",colnames(out.RWF)) ## grab all columns that start with the letter x
ci_1 <- apply(exp(out.RWF[,x.cols]),2,quantile,c(0.025,0.5,0.975)) ## model was fit on log scale

# Plot
out_2 <- as.matrix(ef.out3$predict)
ci_2 <- apply(exp(out_2),2,quantile,c(0.025,0.5,0.975))

plot(time,ci_2[2,],type='n',ylim=range(y,na.rm=TRUE),ylab="Flu Index",log='y',xlim=time[time.rng])

ecoforecastR::ciEnvelope(time,ci_1[1,],ci_1[3,],col=ecoforecastR::col.alpha("purple",0.75))
ecoforecastR::ciEnvelope(time,ci_2[1,],ci_2[3,],col=ecoforecastR::col.alpha("lightBlue",0.75))
lines(time,ci_2[2,],col="lightBlue")

points(time,y,cex=0.5, pch="+", col = is.na(data3$y)+1)#'red') # original points turned into NA
#points(time,data3$y,pch="+",cex=0.5)

legend("topleft", 
       legend = c("NA Points", "Original Data", "Random Walk CI", "Covariates CI"), 
       pch = c("+", "+", NA, NA), 
       col = c("red", "black", NA, NA), 
       pt.bg = c(NA, NA, "purple", "lightblue"), 
       fill = c(NA, NA, "purple", "lightblue"), 
       border = c(NA, NA, "purple", "lightblue"), 
       cex = 1)
...
The DLM model was more accurate and more precise. The confidence interval was closer to the actual data and the spread was less wide. This model could be improved by adding in more covariates, specifically ones that deal with the seasonality of flus.
