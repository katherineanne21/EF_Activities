...
Process error (tau_add) SD = (first) 1.637258 (second)
Observation error (tau_obs) SD = (first) 34.930032 (second)
...
# Visualize
time.rng = c(length(time)-80,length(time))       ## adjust to zoom in and out
out_1 <- as.matrix(jags.out)         ## convert from coda to matrix  
x.cols <- grep("^x",colnames(out_1)) ## grab all columns that start with the letter x
ci_1 <- apply(exp(out[,x.cols]),2,quantile,c(0.025,0.5,0.975)) ## model was fit on log scale

# Plot
out_2 <- as.matrix(ef.out$predict)
