...
According to Table 1, the initial condition uncertainty should change exponentially. This did happen because the black line gets much thinner very quickly. The model is stable and so it tends to its stability point.
...
According table 1, the parameter uncertainty won't average out when aggregated through time. This is shown in our graph because as time increases the red uncertainty stays roughly the same. This is also what I would expect because simply running the model for longer shouldn't affect how certain the model is.
...
You could just sample one parameter and sample each parameter on it's own.
...
According to Table 1, the uncertainty will get greater with time. The green gets wider as time increases. This is because the forecast itself is taking in uncertainty.
...
The process error becomes less important with time (temporal scale). We are averaging out the error over time.
...
## Random effect samples
tau.mc <- 1/sqrt(params[prow,"tau_site"]) ## convert from precision to std deviation
aNew.mc <- rnorm(Nmc,0,mean(tau.mc))            ## draw out-of-sample predictions of alpha at a new site
...
## Plot run
plot.run()
N.IPDEA1.ci = apply(N.IPDEA,2,quantile,c(0.025,0.5,0.975))
ecoforecastR::ciEnvelope(time2,N.IPDEA.ci[1,],N.IPDEA.ci[3,],col=col.alpha(N.cols[5],trans))
ecoforecastR::ciEnvelope(time2,N.IPDEA1.ci[1,],N.IPDEA1.ci[3,],col=col.alpha("lightblue",trans))
ecoforecastR::ciEnvelope(time2,N.IPDE.ci[1,],N.IPDE.ci[3,],col=col.alpha(N.cols[4],trans))
ecoforecastR::ciEnvelope(time2,N.IPD.ci[1,],N.IPD.ci[3,],col=col.alpha(N.cols[3],trans))
ecoforecastR::ciEnvelope(time2,N.IP.ci[1,],N.IP.ci[3,],col=col.alpha(N.cols[2],trans))
ecoforecastR::ciEnvelope(time2,N.I.ci[1,],N.I.ci[3,],col=col.alpha(N.cols[1],trans))
...
The site to site variability is dominating and not the tau site. This is because we don't see a difference.
...
The out-of-sample variance was dominated by Random Effects. One way to change this would be to look at a fixed effect that may be different from site to site (for example soil). The in-sample variance was dominated by process error. The only way to change this is to make a better model. Increasing model complexity would decrease the process error.
