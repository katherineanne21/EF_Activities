...
```{r}
seq(from = -4.8, to = -3.43, length.out = 8) 
```
...
```{r}
write.table(x,"matrix_data.csv",row.names=FALSE)
```
...
```
   ...
   The csv met is a dataframe. It has 8008 entries. It contains columns called time, Shortwave, LongWave, AirTemp, RelHum, WindSpeed, Rain, and Snow. The time column is in the character class and not the date class.
...
```{r}
tadpoles = dat |> filter(tadpoles > 3) |> filter(tadpoles < 5)
head(tadpoles)
```
...
```{r}
frogs = dat |> filter(frogs < 2.5) |> filter(color == "red")
head(frogs)
```
...
```{r}
num14e = dat |> filter(spots == FALSE | frogs > 5)
head(num14e)
```
...
```{r}
met |>  summarise(avg = mean(AirTemp))
```
...
legend("topleft",  ## draw legend in top corner
       ...
       pch = c(1,1,2,2),                      ## matching vectors of plot characters and color
...
```{r}
plot(met$time, met$ShortWave, cex = 0.5, xlab = "Time", ylab = "Shortwave Radiation (Watts)")
```

18. [B] **Plot shortwave (solar) radiation against relative humidity**

```{r}
plot(met$RelHum, met$ShortWave, cex = 0.5, xlab = "Relative Humidity", ylab = "Shortwave Radiation (Watts)")
```
...
met %>%
 filter(time >= "2013-05-28 00:00:00" & time <= "2013-09-03 00:00:00") %>%
 ggplot() +
  aes(x = AirTemp) +
  geom_histogram(bins = 15L, fill = "#FFA6D2") +
  labs(
    x = "Air Temperature (C)",
    y = "Frequency",
    title = "Air Temperature Histogram"
  ) +
  theme_minimal()
```
...
met %>%
 ...
 ggplot() +
  aes(x = AirTemp) +
  geom_density(adjust = 0.4, fill = "#FFA6D2") +
  labs(
    x = "Air Temperature (C)",
    y = "Density",
    title = "Air Temperature Density Plot"
  ) +
  theme_minimal() 
``` 
...
met %>%
 ...
 ggplot() +
  ...
  geom_line(colour = "#911F58") +
  labs(
    x = "Time",
    y = "Air Temperature (C)",
    title = "Air Temperature v Time"
  ) +
  theme_minimal()
```

The major change is that the aes function now has a y = section. Obviously this also means that axis title options have been updated as well.
...
met %>%
 filter(time >= "2013-05-28 00:00:00" & time <= "2013-09-03 00:00:00") %>%
 ggplot() +
  aes(x = time, y = AirTemp, colour = ShortWave) +
  geom_line() +
  scale_color_gradient(low = "#FFA6D2", high = "#84967C") +
  labs(
    x = "Time",
    y = "Air Temperature (C)",
    title = "Air Temperature v Time"
...
The shortwave radiation is highest when the air temperature is highest. This makes sense as the solar radiation will heat up the air. The shortwave peaks at mid-afternoon which also makes sense because that when the sun angle is at it's most direct.
...
```{r}
ggplot(dat) +
  aes(x = tadpoles, y = frogs, colour = color, size = spots) +
  geom_point(shape = "circle") +
  scale_color_manual(
    values = c(blue = "#8DCBFF",
    red = "#FF6161")
  ) +
  labs(
    x = "Tadpoles",
    y = "Frogs",
    title = "Frogs vs Tadpoles"
  ) +
  theme_minimal()
```

This new plot includes a description for color and size and all of the descriptions of that.
...
```{r}
ggplot(dat) +
  aes(x = tadpoles, y = frogs, colour = color) +
  geom_point(shape = "circle", size = 1.55) +
  scale_color_manual(
    values = c(blue = "#8DCBFF",
    red = "#FF6161")
  ) +
