...
```{r}
phenocam = readr::read_csv("https://data.ecoforecast.org/neon4cast-targets/phenology/phenology-targets.csv.gz")
phenocam_harvard = dplyr::filter(phenocam, site_id == "HARV")
plot(phenocam_harvard$datetime, phenocam_harvard$observation, type='l', xlab = "Year", ylab = "GCC 90", main = "Phenocam Phonology Data")
```
...
```{r}
if(file.exists("NOAA_GEFS.Rdata")){  
  load("NOAA_GEFS.Rdata")
} else{  ## to make knit faster, only download the data the first time
  
s3 <- arrow::s3_bucket(bucket = "neon4cast-drivers/noaa/gefs-v12/stage2/parquet/0/2024-01-01",
                       endpoint_override = "data.ecoforecast.org",
                       anonymous=TRUE)
ds <- arrow::open_dataset(s3)

dataframe <- ds |> filter(site_id == "HARV", variable == "air_temperature") |> collect()

save(df,file="NOAA_GEFS.Rdata")
} ## end download


plot(dataframe$datetime, dataframe$prediction, type = "n", xlab = "Date", ylab = "Air Temperature",
     main = "Timeseries of Air Temperature")
index = unique(dataframe$parameter)

for(i in index){
  new_df = filter(dataframe, parameter == i)
  lines(new_df$datetime, new_df$prediction, col = rainbow(length(index))[i], type = 'l')
}
```
...
```{r}
attributes = get_attributes(dt_md$dataTable$attributeList)
attributes_df = attributes$attributes

species_1 <- attributes_df[attributes_df$attributeName == "species_1", ]
species_2 <- attributes_df[attributes_df$attributeName == "species_2", ]

def_1 = species_1$attributeDefinition
def_2 = species_2$attributeDefinition

units_1 = species_1$unit
units_2 = species_2$unit

forecast = eml_get(md$additionalMetadata, "forecast")
process_error = forecast$process_error
process_error_status = process_error$status

# Help from Sebastian
# attributes_spec = subset(attributes_df, attributeName = c("species_1", "species_2"))
```
...
# Check current timestamp of data loaded

# Check metadata of each pixel to see if the temporal coverage ends after the current date

# Download pixels that are new

# Update a working variable of the current timestamp for data loaded

* Write out what the cron table would look like to schedule this job (assume the update only needs to be done weekly)

0 10 * * 0 /path/script.sh
